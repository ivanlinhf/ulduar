...
These gates are independent. Setting the frontend flag without a backend provider makes the UI visible but the image workspace will show an error once it attempts to load generation capabilities. Setting the backend provider without the frontend flag keeps image generation invisible in the UI.
